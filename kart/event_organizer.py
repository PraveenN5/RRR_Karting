    ...
    with tab1:
        ...
        if login_success and event_data:
            ...
            with info_col2:
                ...
                st.write(f"**Status:** {'Live' if event_data['is_live'] == 1 else 'Not Live'}")
                st.write(f"**Complete:** {'Yes' if event_data['is_complete'] == 1 else 'No'}")
                st.write(f"**Closed:** {'Yes' if event_data['is_closed'] == 1 else 'No'}")
    ...
    with tab2:
        ...
        if not is_registration_open():
            ...
            with st.form("event_registration_form"):
                ...
                with date_col2:
                    # Note: These status fields are now controlled by the admin
                    # We set them to default values here
                    is_live = False  # Default to not live
                    is_complete = False  # Default to not complete
                    is_closed = False  # Default to not closed
                    
                    st.info("Event status (Live, Complete, Closed) will be managed by administrators.")
                    ...
                    if errors:
                        ...
                        if success:
                            ...
                            with summary_col2:
                                ...
                                st.write("**Status:** Not Live (Pending admin approval)")
