...
def create_driver_table_if_not_exists():
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS Driver (
            ...
            FOREIGN KEY (event_id, category_id) REFERENCES Category(event_id, category_id)
        )
        ''')
        conn.commit()
        return True
...
def get_available_events():
    ...
    try:
        ...
        query = """
        SELECT event_id, event_name, is_closed 
        FROM Event 
        WHERE is_live = 1 AND is_complete = 0
        """
...
def register_driver(driver_data, profile_pic):
    ...
    (driver_id, driver_name, user_name, country, dob, insta_id, website,
     password_salt, email, phone_number, dp, event_id, category_id)
...
def login_driver():
    ...
    if login_button:
        ...
        try:
            ...
            query = """
            SELECT d.*, e.event_name, c.category_name, e.is_closed, e.is_live, e.is_complete
            FROM Driver d
            LEFT JOIN Event e ON d.event_id = e.event_id
            LEFT JOIN Category c ON d.event_id = c.event_id AND d.category_id = c.category_id
...
def main():
    ...
    if not create_driver_table_if_not_exists():
        st.error("Failed to create or verify Driver table. Please check your connection.")
        st.stop()
    ...
    with tab1:
        ...
        if login_success and driver_data:
            ...
            with info_col2:
                ...
                if driver_data['category_name']:
                    st.write(f"**Category:** {driver_data['category_name']}")
            
            # Display event status information
            st.subheader("Event Status")
            if driver_data['is_complete'] == 1:
                st.warning("⚠️ This event has been completed.")
            elif driver_data['is_live'] == 0:
                st.info("ℹ️ This event is not live yet.")
            elif driver_data['is_closed'] == 1:
                st.warning("⚠️ Registration for this event is currently closed.")
            else:
                st.success("✅ This event is live and accepting registrations.")
    ...
    with tab2:
        ...
        if not available_events:
            st.warning("⚠️ There are no active events available at this time. Please check back later.")
        else:
            st.subheader("Register as a Driver")
            st.write("Complete the form below to register as a racing driver.")
            
            # Show all live events, but indicate which ones have registration closed
            st.write("### Available Events")
            event_status_df = pd.DataFrame([
                {"Event ID": e['event_id'], 
                 "Event Name": e['event_name'], 
                 "Registration Status": "Closed" if e['is_closed'] == 1 else "Open"}
                for e in available_events
            ])
            st.dataframe(event_status_df, use_container_width=True)
            
            # Event selection outside the form to trigger category loading
            st.write("### Event Selection")
            open_events = [e for e in available_events if e['is_closed'] == 0]
            
            if not open_events:
                st.warning("⚠️ All events currently have registration closed. Please check back later.")
                st.stop()
                
            event_options = {event['event_id']: f"{event['event_name']} (ID: {event['event_id']})" for event in open_events}
            ...
            if not categories:
                st.warning("⚠️ This event has no categories defined. Please select another event or contact the administrator.")
            else:
                with st.form("driver_registration_form"):
                    # Category selection
                    st.write("### Category Selection")
                    category_options = {cat['category_id']: cat['category_name'] for cat in categories}
                        ...
                        if errors:
                            ...
                            if success:
                                ...
                                with summary_col2:
                                    ...
                                    st.write(f"**Category:** {category_options[selected_category_id]}")
                            else:
                                st.error(message)
